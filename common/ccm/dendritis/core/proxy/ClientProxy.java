...
public class ClientProxy extends CommonProxy {
    ...
    public void handleTileEntityPacket(final int x, final int y, final int z, final ForgeDirection orientation, final short state, final String owner, final String customName) {
        ...
        if (tileEntity != null) {
            /*
             * if (tileEntity instanceof TileBase){ ((TileBase) tileEntity).setOrientation(orientation); ((TileBase) tileEntity).setState(state); ((TileBase)
             * tileEntity).setOwner(owner); ((TileBase) tileEntity).setCustomName(customName); }
             */
        }
    }
}